...
export const departmentGroupService = {
	list: () => db.select().from(departmentGroup).orderBy(asc(departmentGroup.sortOrder), asc(departmentGroup.name)),

	getById: (id: number) => db.select().from(departmentGroup).where(eq(departmentGroup.id, id)).then((r) => r[0]),

	create: (data: { name: string; sortOrder?: number }) =>
		db.insert(departmentGroup).values(data).returning().then((r) => r[0]),

	update: (id: number, data: { name?: string; sortOrder?: number }) =>
		db.update(departmentGroup).set(data).where(eq(departmentGroup.id, id)).returning().then((r) => r[0]),
	...
	listWithDepartments: async () => {
		const groups = await db.select().from(departmentGroup).orderBy(asc(departmentGroup.sortOrder), asc(departmentGroup.name));
		...
		for (const m of members) {
			...
			memberMap.get(m.groupId)!.push({ departmentId: m.departmentId, departmentName: m.departmentName });
		}
	...
	addDepartment: (groupId: number, departmentId: number) =>
		db.insert(departmentGroupMember).values({ groupId, departmentId }).onConflictDoNothing().returning().then((r) => r[0]),

	/** Удалить отдел из группы */
	removeDepartment: (groupId: number, departmentId: number) =>
		db.delete(departmentGroupMember).where(and(eq(departmentGroupMember.groupId, groupId), eq(departmentGroupMember.departmentId, departmentId)))
};
