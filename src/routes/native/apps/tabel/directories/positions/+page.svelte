<script lang="ts">
	...
	const clearUrl = $derived(data.search ? '?search=' : '/native/apps/tabel/directories/positions');
</script>

{#if canEdit}
	<Card title="Новая должность">
...
<Card title={`Должности (${data.positions.length})`}>
	<form method="get" action="/native/apps/tabel/directories/positions" class="n-filters">
		...
		<table class="native-table n-table">
			...
			<tbody>
				{#each data.positions as p}
					<tr>
						<td class="cell cell-left">{p.name}</td>
						{#if canEdit}
							<td class="cell n-actions">
								<form method="post" action="?/update" class="n-inline-form n-edit">
									<input type="hidden" name="id" value={p.id} />
									<input type="text" name="name" value={p.name} class="n-edit-input" />
