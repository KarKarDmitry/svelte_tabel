<script lang="ts">
	...
	let fetched = $state<{ departments?: any[]; search?: string }>({});

	let departments = $derived(fetched.departments ?? data.departments);
	...
	async function onSearch(value: string) {
		...
		if (res.ok) {
			...
			fetched = { departments: json.departments, search: value };
		}
	}
