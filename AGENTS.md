...
- **Вьювер для Windows XP** (`client/tabel-viewer/`, сборка готова `tabviewer.exe`): автономный нативный C (Win32, MinGW, ноль установок на XP) — только просмотр табеля + диалог «События сотрудника». Серверный JSON-endpoint `GET /apps/tabel/tabel/month?year=&month=` создан (`src/routes/apps/tabel/tabel/month/+server.ts`, тонкая обёртка над `tabelMonthData`; compile-check 0 ошибок). Код клиента: `main.c` (окно, ListView, навигация, логин), `events.c` (диалог событий, `GET …/employee-events`), `net.c` (WinINet, cookie-джар как единственный источник — ручной Cookie НЕ передаём), `model.c` (порт `cell-style.ts` для light-правил: markRule→missingHours→weekendWork→over/underwork→missedWorkday; учет `shiftMarks`/`isShift`), `json.c/h` (мини JSON DOM). Сборка: `build.cmd` — `C:\mingw32\bin\gcc.exe` (winlibs **i686 + msvcrt**, НЕ ucrt/x64!). ✅ **Скомпилировано** (gcc 16.2, i686, `-Wall -Wextra` 0 предупреждений; `objdump -p` — только системные DLL: msvcrt/kernel32/user32/gdi32/comctl32/wininet). Осталось: живой тест против сервера + проверка на XP. Нюансы: `INTERNET_FLAG_NO_AUTO_REDIRECT` (иначе 302 auth-guard → HTML логина); `lastDay`-дни без записей инициализируются `-1` (не 0); кодировка `-fexec-charset=CP1251` (ANSI _A_-функции) — в кнопках нельзя `◀▶` (нет в CP1251), ставить `<`/`>`; `build.cmd` сам делает `cd /d %~dp0`. .gitignore: exe/obj/o/token.txt в `client/tabel-viewer/`.
