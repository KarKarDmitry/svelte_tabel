...
16. Крупные файлы: `T-12_builder*.ts` (ок. 950), `tabel/+page.svelte` (912), `worktime.service.ts` (644), `turnstile-import.service.ts` (ок. 1200) — декомпозировать.
...
- Прод-схема: nginx (`nginx.conf`, порт 80) проксирует на `app:3000`; `proxy_buffering off` — для SSE статуса импорта. TLS-блок в `nginx.conf` закомментирован — включить вместе с сертификатом (приватный CA, `mettem.lan`). `dnsmasq.conf` готов, сервис `dns` в `compose.yaml` закомментирован.
...
- Прод-деплой: сгенерировать сертификат (приватный CA, `mettem.lan`), включить TLS-блок в `nginx.conf` и порт 443 в `compose.yaml`, подключить `certs` volume; поднять сервис `dns` (dnsmasq) — конфиг `dnsmasq.conf` готов.
- Открытые пункты ревью: архитектурные п.14-18 (дублирование деревьев, баг native POST на employee-events, крупные файлы, worktime.service.ts, тесты).
- **Сменить пароли** (критично): MSSQL `Editor` и PG-пароль из старой истории **всё ещё рабочие** — сменить в БД и в `.env` (см. ниже).
- Выполнено (закоммичено): харденинг безопасности (п.1-7, 10-13), слой прав на чтение (`29e399a`), native-увольнение (`227d099`), скиллы opencode, `check:compile`, рефакторинг импорта + безопасность экспорта/статуса/`/admin` + pino-логгер + профайлер + N+1-фикс (`b2db032`), фикс поиска сотрудников (`12f0a03`).
