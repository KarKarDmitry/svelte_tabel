...
- **Кэш-сервис (согласованный план, in-process без Redis — деплой одноузловой):**
  - Ядро `src/lib/server/cache.ts`: `remember(key, ttl, tags, loader)` + single-flight; `invalidate(...tags)`; `cache.update(key, mutator)` — точечная правка закэшированного объекта (нет ключа → no-op); LRU-лимит ~500; `stats()`; отключение через `CACHE_ENABLED=0`.
  - S1 (справочники): day-marks, константы (`const:<KEY>`), графики/календари, departments/positions — TTL 300с; интеграция в `buildStyles` и лоады справочников. Инвалидация из мутаций сервисов (не контроллеров).
  - S2 (месяц): `getMonthGrouped` → `wtt-month:<y>-<m>`, TTL 900с. Точечный патч `patchWttMonth` (день сотрудника + пересчёт итогов) из `updateDayMark`/bulk/`saveEmployeeEvents`; импорт турникетов на done — групповой сброс тега. Python-импорт игнорируем (разовый перенос).
